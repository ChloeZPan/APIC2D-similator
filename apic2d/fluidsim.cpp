                          ...
                          bool draw_grid_, bool draw_particles_, bool draw_velocities_, bool draw_boundaries_)
{
  ...
  m_sorter = new sorter(ni, nj);
    // [modified]
    myfile.open("/Users/zeyipan/Documents/20FALL/Research/buildcode/apic2d/output.txt");
}
  ...
  for(int i = 0; i < ni; ++i)
  {
    for(int j = 0; j < nj; ++j) {
      for(int k = 0; k < 2; ++k) {
        ...
        Vector2s pt = Vector2s(x,y) + Vector2s(0, 0);
          scalar phi = compute_phi(pt);
//          if(phi > dx * 20.0 && particles.size() < 5) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
          if(phi > dx * 20.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
          
          // [modified]
//           (1) 50 50 10
//          Vector2s pt = Vector2s(x,y) + Vector2s(50, 50);
//
//        scalar phi = compute_phi(pt);
//        if(phi > dx * 10.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
          
          // (2) - (50, 50, 10)
//          Vector2s pt = Vector2s(50, 50) - Vector2s(x,y);
//
//        scalar phi = compute_phi(pt);
//        if(phi > dx * 10.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
          
        // [modified] 505010 upper left
//      Vector2s pt = Vector2s(-x,y) + Vector2s(50, 50);
//      scalar phi = compute_phi(pt);
//      if(phi > dx * 10.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
...
//        if(phi > dx * 10.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
          ...
          // 454515 upper right
//          Vector2s pt = Vector2s(x,y) + Vector2s(45, 45);
//          scalar phi = compute_phi(pt);
//          if(phi > dx * 15.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
          // 454515 upper left
//        Vector2s pt = Vector2s(-x,y) + Vector2s(0, 0);
//        scalar phi = compute_phi(pt);
//        if(phi > dx * 20.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
          
//          Vector2s pt = Vector2s(x+50,y) + Vector2s(0, 0);
//          scalar phi = compute_phi(pt);
//          if(phi > dx * 20.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
          
//          Vector2s pt = Vector2s(x,y+50) + Vector2s(0, 0);
//          scalar phi = compute_phi(pt);
//          if(phi > dx * 20.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
//
//          Vector2s pt = Vector2s(x,y) + Vector2s(50, 20);
//          scalar phi = compute_phi(pt);
//          if(phi > dx * 10.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
//        Vector2s pt = Vector2s(x,y) + Vector2s(40, 10);
//        scalar phi = compute_phi(pt);
//        if(phi > dx * 10.0) add_particle(Particle(pt, Vector2s::Zero(), dx / sqrt(2.0), PT_LIQUID));
      }
    }
...
void FluidSim::writein() {
//    myfile << "center:" << root_boundary->center << "\t";
//    myfile << "p1:" << root_boundary->parameter[0] << "\t";
//    myfile << "p2:" << root_boundary->parameter[1] << std::endl;
     for (int i=0; i < particles.size(); i++) {
         myfile << particles[i].x[0] << "\t";
         myfile << particles[i].x[1] << "\t";
         myfile << particles[i].v[0] << "\t";
         myfile << particles[i].v[1] << " | ";
     }
    myfile << std::endl;
}
